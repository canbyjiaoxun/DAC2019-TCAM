...
inline void tiny_quantized_fully_connected_error_kernel(
    ...
    for_i(layer_parallelize, params.out_size_, [&](int i) {
      for (serial_size_t c = 0; c < params.in_size_; c++) {
        ...
        //  static_cast<int32_t>(in_quantized[c] - offset_input);
      	int32_t temp1 = static_cast<int32_t>(W_quantized[c * params.out_size_ + i ] - offset_filter );
	temp1= tiny_dnn::kernels::FlipAdder32(temp1);
	int32_t temp2 = static_cast<int32_t>(in_quantized[c] - offset_input);
	temp2= tiny_dnn::kernels::FlipAdder32(temp2);
	int32_t product = temp1 * temp2; 
	product = tiny_dnn::kernels::FlipMulti32(product);
	a_quantized[i] = a_quantized[i] + product;
	a_quantized[i] = tiny_dnn::kernels::FlipAdder32(a_quantized[i]);
	}
      if (params.has_bias_) {
	int32_t diff = bias_quantized[i] - zero_in_total_space;
	diff = tiny_dnn::kernels::FlipAdder32(diff);
	a_quantized[i] = a_quantized[i] + diff;
	a_quantized[i] = tiny_dnn::kernels::FlipAdder32(a_quantized[i]);
