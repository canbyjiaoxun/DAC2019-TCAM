...
inline void tiny_quantized_conv2d_error_kernel(const conv_params &params,
                                         const vec_t &in,
                                         const vec_t &W,
                                         const vec_t &bias,
                                         vec_t &a,
  ...
  for_i(layer_parallelize, params.out.depth_, [&](int o) {
    for (serial_size_t inc = 0; inc < params.in.depth_; inc++) {
      ...
      for (serial_size_t y = 0; y < params.out.height_; y++) {
        for (serial_size_t x = 0; x < params.out.width_; x++) {
	      ...
	      int32_t temp1 = (static_cast<int32_t>(*ppw++) - offset_filter);
	      temp1= tiny_dnn::kernels::FlipAdder32(temp1);
	      int32_t temp2 = (static_cast<int32_t>(ppi[idx]) - offset_input);	
              temp2= tiny_dnn::kernels::FlipAdder32(temp2);
	      int32_t product = temp1*temp2;  
	      product = tiny_dnn::kernels::FlipMulti32(product);
	      sum += product;
	      sum = tiny_dnn::kernels::FlipAdder32(sum);
            }
          }
          pa_quantized[y * params.out.width_ + x] += sum;
	  pa_quantized[y * params.out.width_ + x] = tiny_dnn::kernels::FlipAdder32(pa_quantized[y * params.out.width_ + x]);
	...
	int32_t diff = bias_quantized[o] - zero_in_total_space;
	diff = tiny_dnn::kernels::FlipAdder32(diff);
	f = f + diff;
	f = tiny_dnn::kernels::FlipAdder32(f);
      });
    }
  });
