...
#include "tiny_dnn/core/kernels/tiny_quantized_conv2d_error_kernel.h"

#ifdef CNN_USE_GEMMLOWP
#include "tiny_dnn/core/kernels/tiny_quantized_fully_connected_kernel.h"
#include "tiny_dnn/core/kernels/tiny_quantized_fully_connected_error_kernel.h"
...
class tiny_backend : public backend {
 public:
  ...
  // quantized convolution error
  void conv2d_qe(const std::vector<tensor_t *> &in_data,
                std::vector<tensor_t *> &out_data) override {
    copy_and_pad_input(*in_data[0]);
    const vec_t &W    = (*in_data[1])[0];
    const vec_t &bias = (*in_data[2])[0];
    ...
    for (serial_size_t i = 0; i < in.size(); i++) {
      kernels::tiny_quantized_conv2d_error_kernel(*params_c_, *in[i], W, bias, a[i],
                                            layer_->parallelize());
    }
  }
  ...
  void conv2d_eq(const std::vector<tensor_t *> &in_data,
    ...
    fill_tensor(a, float_t{0});
    for (serial_size_t i = 0; i < in.size(); i++) {
      kernels::tiny_quantized_conv2d_error_kernel(*params_c_, *in[i], W, bias,
  ...
  void fully_qe(const std::vector<tensor_t *> &in_data,
               std::vector<tensor_t *> &out_data) override {
#ifdef CNN_USE_GEMMLOWP
    ...
    for (serial_size_t i = 0; i < in.size(); i++) {
      kernels::tiny_quantized_fully_connected_error_kernel(
